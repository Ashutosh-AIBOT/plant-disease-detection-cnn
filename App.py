...
.report-header {
    background: linear-gradient(90deg, #0f2a1a, transparent);
...
# --------------------------
# DASHBOARD HEADER (Matched to app01.py)
# --------------------------
st.markdown("""
...
def handle_diagnostic(image_file, file_id):
    ...
    if conf < 0.40:
        response = "⚠️ **Result:** No match found in our database. The leaf might be too blurry or is an unsupported variety."
    else:
        with st.spinner("Retrieving Treatment Data..."):
            context = get_rag_context(vectorstore, label)
            prompt = f"User uploaded a plant leaf image. Model detected: {label} ({conf*100:.1f}% confidence).\nContext: {context}\nProvide a professional diagnostic report with Treatment and Prevention."
            ai_resp = llm.invoke(prompt)
            response = f"### 🔍 Diagnostic: {label.replace('___',' — ').replace('_',' ')}\n\n" + ai_resp.content
...
if action_mode == "📁 UPLOAD LEAF":
    ...
    if file: handle_diagnostic(file, f"{file.name}_{file.size}")

elif action_mode == "📷 LIVE SCAN":
    cam = st.camera_input("Capture Scan", label_visibility="collapsed")
    if cam: handle_diagnostic(cam, f"cam_{int(time.time())}")

# Continuous Chat
if prompt := st.chat_input("Ask about plant health or treatment..."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            ...
            rag = get_rag_context(vectorstore, prompt)
